 ...
 * graph: recursive {slug, name, level, instances, children}. Sibling pivot
 * rows pointing at the same child collapse into one node whose `instances`
 * count drives the "×n → #1 #2 #3" chips. Depth-capped at
...
{
    /**
     * @return array{slug: string, name: string, level: string, instances: int, children: list<array>}
     */
     ...
     * @return list<array{slug: string, name: string, level: string, instances: int, children: list<array>}>
     */
