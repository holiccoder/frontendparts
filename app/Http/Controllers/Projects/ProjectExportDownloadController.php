 ...
 * §8.6): streams a built pack zip from the private exports disk (owner-only,
 * scoped bindings) and records a `download` component event for every
 * component in the pack — the same license download tracking the
...
class ProjectExportDownloadController extends Controller
{
    public function __invoke(Request $request, Project $project, ProjectExport $export): StreamedResponse
    {
        ...
        $project->components()->get()->each(
            fn (Component $component) => $component->recordEvent(ComponentEventType::Download, $request->user())
        );
