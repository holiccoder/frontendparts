 ...
 * `import.meta.glob` and pairs each `index.vue` with its `data.json`.
...
const componentModules = import.meta.glob<{ default: Component }>('../components/**/index.vue', {
    eager: true,
});
...
/** `../components/elements/section-title-01/index.vue` → `elements/section-title-01` */
function slugFromPath(path: string): string {
    return path.replace(/^\.\.\/components\//, '').replace(/\/index\.vue$/, '').replace(/\/data\.json$/, '');
}
