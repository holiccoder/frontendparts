/**
 * Browser smoke suite — Playwright driving the SYSTEM Google Chrome
 * (`channel: 'chrome'`; no chromium download, no ChromeDriver to
 * version-match). The skeleton keeps ONE smoke spec (home renders); add
 * product specs next to it as the product grows.
 *
 * HOW TO RUN
 *   npm run test:browser          # build the app, rebuild browser.sqlite, run headless
 ...
 *      - wipes database/browser.sqlite and rebuilds it via migrate:fresh --seed
 ...
 *   `php artisan test`. Artifacts (results/, report/, .cache/) are gitignored.
 */
