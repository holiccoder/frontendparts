---
title: Legal Notice
description: Operator identity and contact details, and how Paddle appears as merchant of record on your invoices.
updated: 2026-07-21
---

# Legal Notice

This page identifies the operator of the Service and provides the legally required contact information (imprint-style notice, e.g. under German §5 DDG and equivalent EU rules).

## Operator

**[Product / brand name]**

[Operator legal name — to be added before public launch]
[Registered street address]
[Postal code, city, country]

- Contact: open a [support ticket](/dashboard/tickets/new) or email [operator contact email — to be added before public launch]
- Commercial register / company number: [to be added if applicable]
- VAT ID: [to be added if applicable]
- Responsible for content under applicable press law: [name, address — to be added if applicable]

The bracketed details are finalized when the operating entity is registered.

## Merchant of record

Purchases are processed by **Paddle.com** as our merchant of record. This means:

- Paddle — not the operator — appears as the seller on your card statement, receipts, and invoices;
- Paddle collects and remits applicable sales taxes (VAT, GST, sales tax) as seller;
- payment data is handled under Paddle's own buyer terms and privacy policy, presented at checkout.

Your contractual relationship for the Service itself is with the operator named above; your payment transaction is with Paddle.

## Dispute resolution

The European Commission provides an Online Dispute Resolution platform. We are neither obliged nor willing to participate in dispute-resolution proceedings before a consumer arbitration board, but we will always try to resolve issues directly — the [support ticket system](/dashboard/tickets/new) reaches us fastest.

## Liability for content and links

The Service may link to external sites we do not control; their operators are solely responsible for them. Links were checked when published — if you find one that is broken or points at changed, problematic content, tell us via a [support ticket](/dashboard/tickets/new) and we will fix it.

See also: [Terms of Service](/terms) · [Privacy Policy](/privacy) · [Copyright & Takedown Policy](/copyright)
